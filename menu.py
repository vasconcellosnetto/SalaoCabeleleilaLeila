...
class Application:
    ...
    def __init__(self, master=None):
        ...
        self.usuario = Entry(self.segundoContainer, width = 20, font = self.fontePadrao)
    ...
    def verificaSenha(self):
        usuario = self.db.teste_login(self.usuario.get(), self.senha.get())
        if self.db.teste_login(self.usuario.get(), self.senha.get()) is not None:
            print(usuario)
            root.destroy()
        else:
